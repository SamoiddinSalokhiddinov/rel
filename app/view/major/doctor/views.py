...
from common.admittance.models import AdmittanceType, Admittance , Symptoms , ChronicIllness
from .form import *
...
class DoctorDetailView(SuccessMessageMixin, DetailView):
    ...
    def post(self, request , pk):
        ...
        doc = Doctor.objects.filter(id=self.kwargs['pk']).first()
        # print(request.POST)
        form = AdmittanceForm(request.POST)
        form.data['doctor'] = doc.pk
    ...
    def get_context_data(self, *args ,**kwargs):
        ...
        form = AdmittanceForm()
        # form = AdmittanceForm(pk=self.kwargs['pk'])
...
class SymptomDetailView(SuccessMessageMixin, DetailView):
    model = Doctor
    template_name = 'doctor/symptoms.html'
    context_object_name = 'doctor'
    slug_field = "pk"
    ...
    def get_context_data(self, *args ,**kwargs):
        ...
        queryset = Doctor.objects.prefetch_related(
            ...
            Prefetch(
                "symp_doctor",
                queryset = Symptoms.objects.filter(doctor=self.kwargs["pk"]).order_by("id"),
                to_attr="symptoms"
            )
        ).filter(id=self.kwargs['pk']).first()
        admittance_types = queryset.admittance_types
        admittances = queryset.admittances
        symptoms = queryset.symptoms
...
class SyptomCreateView(SuccessMessageMixin, DetailView):
    model = Doctor
    template_name = 'doctor/sym_create.html'
    context_object_name = 'doctor'
    slug_field = "pk"
    ...
    def post(self, request , pk):
        ...
        if form.is_valid():
            form.save()
            messages.success(request, _("Symptom successfully added to the list'"))
            return HttpResponseRedirect(reverse_lazy("doctor:symptoms" , kwargs={"pk" : pk}))
        else:
            form = AdmittanceForm()
            return HttpResponseRedirect(reverse_lazy("doctor:symptoms",  kwargs={"pk" : pk}))
